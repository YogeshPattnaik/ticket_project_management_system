...
if (typeof window !== 'undefined') {
  ...
  if ((window as any).__REACT__ && (window as any).__REACT_DOM__) {
    ...
    console.log('[analytics-mfe] Using host React (immediate)');
  } else {
    ...
    console.log('[analytics-mfe] Using local React (host React not available yet)');
    
    // Try to get host React after a short delay
    setTimeout(() => {
      if ((window as any).__REACT__ && (window as any).__REACT_DOM__) {
        console.log('[analytics-mfe] Host React now available, but already initialized with local React');
      }
    }, 100);
  }
} else {
...
const lifecycles = singleSpaReact({
  ...
  errorBoundary(err, info, props) {
    return React.createElement('div', { className: 'p-4 bg-red-50 border border-red-200 rounded' },
      React.createElement('h2', { className: 'text-red-800 font-bold' }, 'Error in Analytics MFE'),
      React.createElement('p', { className: 'text-red-600' }, err.message)
    );
  },
  domElementGetter: () => {
    let el = document.getElementById('analytics-mfe-container');
    if (!el) {
      el = document.createElement('div');
      el.id = 'analytics-mfe-container';
